...
export type InputStream = UploadHandlerPart & { stream?: Readable, text?: string };
export type InputData = InputStream | null | undefined;
export type Input = Readable | string | null | undefined;
export declare type UploadHandler = (part: UploadHandlerPart) => Promise<InputData>;

export const uploadHandler: UploadHandler = async (part: UploadHandlerPart): Promise<InputData> => {
    if (!part?.data) {
        return undefined;
    }
